...
- You may run a build when needed, but do not use parallel compilation flags such as `-j`. Use serial or keep-going builds only when explicitly required, and prefer `-k` over parallel execution.
